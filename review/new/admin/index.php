<?php
    if (((isset($_COOKIE['name']))and(isset($_COOKIE['level'])))==false){
        echo '<script type="text/javascript">alert("啊呀呀，本功能只开放给网站管理员呢~~~~~");</script>';
...
<body>
    ...
    <br>
    <br>
        <form name="new_review_a" method="POST" action="../../../function_lib//new_r_a.php" onsubmit="return mycheak()">
        <p>帖子标题<input type="text" name="title"></p>
        ...
        <input type="submit" value="发布管理帖子">
        </p>
        </form>
      ...
      <script>
        function mycheak(){
            if  (new_review_a.title.value==""){
              alert("你的帖子的标题在哪？");return false;
            }
            if (new_review_a.review.value=="<p><br></p>"){
